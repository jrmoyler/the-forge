...
create function public.inspect_invite(token text) returns text language sql security definer set search_path=public,extensions as $$ select role from invites where token_hash=encode(digest(token,'sha256'),'hex') and (reusable or used_at is null) and expires_at>now() $$;
